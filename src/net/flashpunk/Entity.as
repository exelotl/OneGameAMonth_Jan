package net.flashpunk
{
	...
	public class Entity extends Tweener
	{
		...
		/**
		 * Game-specific flags, to determine how other entities react to this one.
		 * Make a file full of static constants to store your game's flags.
		 */
		...
		// Component-related code (geckojsc):
		...
		public function removeComponent(name:String):void {
			var comp:Component = _components[name];
			if (comp) {
		...
		/// Called by the World class update loop, no need to call explicitly.
		...
		private function moveCollideCompsX(e:Entity):Boolean {
			if (_components === null) return false;
			for each (var comp:Component in _components)
				if (comp.active) comp.moveCollideX(e)
			return moveCollideX(e);
		}
		
		/// Call moveCollideY for the entity and its components
		private function moveCollideCompsY(e:Entity):Boolean {
			if (_components === null) return false;
			for each (var comp:Component in _components)
				if (comp.active) comp.moveCollideY(e)
		...
		/** @private */ protected var _components:Object;
	}
}
